...
public class SpaceshipController : MonoBehaviour
{
    ...
    public void Move()
    {
        ...
        if(variableJoystick != null)
            Turn(variableJoystick.Horizontal);
        ...
        switch (currentWeapon)
        {
            case Weapon.Basic:
                ProjectileController projectile =
                    Instantiate(weaponList.weapons[(int)currentWeapon].weaponPrefab, transform.position,
                        Quaternion.identity).GetComponent<ProjectileController>();
                projectile.SetDirection(transform.up);
                break;
            case Weapon.Laser:
                if (weaponInstance != null)
                    break;
                weaponInstance = Instantiate(weaponList.weapons[(int)currentWeapon].weaponPrefab,
    ...
    private void CalculatePositionOnCamera()
    {
        if (Camera.main != null)
        {
            ...
            if (positionOnCamera.x > 1.05f)
            {
                warped = true;
                transform.position =
                    (Vector2)Camera.main.ViewportToWorldPoint(new Vector2(-0.05f, positionOnCamera.y));
            }
            else if (positionOnCamera.x < -0.05f)
            {
                warped = true;
                transform.position =
                    (Vector2)Camera.main.ViewportToWorldPoint(new Vector2(1.05f, positionOnCamera.y));
            }
            else if (positionOnCamera.y > 1.05f)
            {
                warped = true;
                transform.position =
                    (Vector2)Camera.main.ViewportToWorldPoint(new Vector2(positionOnCamera.x, -0.05f));
            }
            else if (positionOnCamera.y < -0.05f)
            {
                warped = true;
                transform.position =
                    (Vector2)Camera.main.ViewportToWorldPoint(new Vector2(positionOnCamera.x, 1.05f));
            }
