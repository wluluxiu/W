...
public class GameManager : MonoBehaviour 
{
    ...
    public GameObject variableJoystickPrefab;
    ...
    void Start()
    {
        if (startEnabled)
        {
            GameObject InGameMenuObj = GameObject.Find("InGameMenu");
            if(InGameMenuObj != null)
                variableJoystick = Instantiate(variableJoystickPrefab, InGameMenuObj.transform,false).GetComponent<VariableJoystick>();
            spaceship = Instantiate(spaceshipPrefab, Vector3.zero, Quaternion.identity).GetComponent<SpaceshipController>();
            if(variableJoystick != null)
                spaceship.variableJoystick = variableJoystick;
            if (Camera.main != null)
            {
                cameraController = Camera.main.gameObject.AddComponent<CameraController>();
                cameraController.ChangeCharacter(spaceship);
            }
        }
    }
    ...
    public void SpawnAsteroids()
    {
        ...
        if (Camera.main != null)
        {
            Vector2 spawnPosition = Random.value > 0.5f ? 
                Camera.main.ViewportToWorldPoint(new Vector2(Random.value > 0.5f ? Random.Range(-0.1f, -0.05f) : 
                    Random.Range(1.05f, 1.1f), Random.Range(-0.1f, 1.1f))) : 
                Camera.main.ViewportToWorldPoint(new Vector2(Random.Range(-0.1f, 1.1f), Random.value > 0.5f ? 
                    Random.Range(-0.1f, -0.05f) : Random.Range(1.05f, 1.1f)));
            asteroid = Instantiate(asteroidPrefab, spawnPosition, Quaternion.identity).GetComponent<AsteroidController>();
            asteroid.spaceship = spaceship;
