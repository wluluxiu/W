namespace jj.TATools.Editor
{
    ...
    internal class SceneNodeData
    {
        ...
        internal SceneNodeData m_Father;
        ...
        string GetRelativeNodePath(SceneNodeData father)
        {
    ...
    internal class SceneDetection : BaseDetection
    {
        #region Fields
        ...
        internal List<SceneNodeData> m_LossDataList;
        ...
        internal override void DoDetection(string assetPath)
        {
            ...
            Dictionary<string, SceneNodeData> totalNodeMapping = new Dictionary<string, SceneNodeData>();
            SceneNodeData sceneNodeData = null;
            ...
            while (lineStr != null)
            {
                ...
                if (lineStr == GAMEOBJECT_PROP)
                {
                    ...
                    sceneNodeData = new SceneNodeData();
                    totalNodeMapping.Add(fileID, sceneNodeData);
                }
                // GameObject Name
                else if (collectGameObjectFlag && lineStr.StartsWith(GAMEOBJECT_NAME_PROP))
                {
                    sceneNodeData.m_NodeName = lineStr.Replace(GAMEOBJECT_NAME_PROP, "");
                }
                // GameObject Component Start
                else if (collectGameObjectFlag && lineStr == GAMEOBJECT_COMP_PROP)
                {
                    sceneNodeData.m_CompenentFileIDS = new List<string>();
                }
                ...
                else if (collectGameObjectFlag && lineStr.StartsWith(GAMEOBJECT_COMP_ITEM_START_PROP))
                {
                    ...
                    if (arr.Length == 2)
                    {
                        ...
                        {
                            sceneNodeData.m_CompenentFileIDS.Add(componentFileID);
                ...
                else if (lineStr.StartsWith(PREFAB_LINK_ADD_OBJ_START_PROP))
                {
                    ...
                    if (arr.Length == 2)
                    {
                        ...
                        {
                            if (sceneNodeData.m_CompenentFileIDS == null) sceneNodeData.m_CompenentFileIDS = new List<string>();
                            sceneNodeData.m_CompenentFileIDS.Add(componentFileID);
                            componentID2GoMapping[componentFileID] = fileID;
                        }
                    }
                ...
                else if (collectLinkPrefabName && lineStr.StartsWith(PREFAB_LINK_NAME_VALUE_PROP))
                {
                    ...
                    sceneNodeData.m_NodeName = lineStr.Replace(PREFAB_LINK_NAME_VALUE_PROP, "");
                }
            ...
            foreach (var data in totalNodeMapping.Values)
            {
                if (data.Invalid)
                {
                    if (this.m_LossDataList == null) this.m_LossDataList = new List<SceneNodeData>();
