			...
			foreach(var ag in incoming) {
				foreach(var groupKey in ag.assetGroups.Keys) {
					...
					foreach(var asset in assets) {
                        ...
                        if (AssetGraphPostprocessor.Postprocessor.HasModifiedByOtherGraph(asset))
                        {
                            LogUtility.Logger.Log($"{asset.importFrom} is modified by another graph, Node [{node.Name}] will not process this asset");
                            continue;
                        }
                        ...
                        AssetGraphPostprocessor.Postprocessor.AddModifiedImportSetting(asset);
					}
				}
			}
		}
