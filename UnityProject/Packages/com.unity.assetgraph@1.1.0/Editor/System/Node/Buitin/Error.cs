...
namespace UnityEngine.AssetGraph {
    ...
    public class Error : Node {
        ...
        [SerializeField] private bool m_onlyLog = false;
    	...
    	public override void OnInspectorGUI(NodeGUI node, AssetReferenceStreamManager streamManager, NodeGUIEditor editor, Action onValueChanged) {
            ...
            var onlyLog = EditorGUILayout.ToggleLeft("Not Throw Exception", m_onlyLog);
            if (onlyLog != m_onlyLog)
            {
	            using(new RecordUndoScope("Change Only log", node, true)) {
		            m_onlyLog = onlyLog;
		            onValueChanged();
	            }
            }
            ...
            if(incoming != null) {
                foreach(var ag in incoming) {
                    foreach (var assets in ag.assetGroups.Values) {
                        if (assets.Count > 0) {
	                        Debug.LogError($"{m_description},{m_howtoFix},{assets[0].importFrom}");
	                        if (!m_onlyLog)
	                        {
